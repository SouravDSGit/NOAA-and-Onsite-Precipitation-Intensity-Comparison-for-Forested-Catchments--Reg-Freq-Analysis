...
library(zoo)
library(pracma)
#files=list.files('G:/Sourav/USFS/Revised/HJ Andrews/data_for_Reg_FA/NOAA/NOAA_Oregon_results/final/')
dur=list.files('D:/Sourav/USFS/Revised/Coweeta/Annual_Maxima_Series/RG06/')
pf=list()
...
for (c in 1:length(cld)){
  ...
  for (i in 1:length(clfiles)){
    print(i)
    datae=read.table(paste0('D:/Sourav/USFS/Revised/Coweeta/Annual_Maxima_Series/',clfiles[i],'/',dur[d]))
    datae1=datae[which(datae[,1]>2015),]
    datai=read.table(paste0('D:/Sourav/USFS/Revised/Coweeta/Annual_Maxima_Series_csv/',clfiles[i]))
    data1=cbind(datai[,1],datai[,d+1])
    data1=rbind(data1,datae1)
...
RG=lmomobj$name
#write.table(outg,paste0("G:/Sourav/USFS/Revised/Coweeta/RFA_results/",RG[s],"/",dur[d]),row.names=FALSE, col.names=FALSE,na = "NaN")
#write.table(outgf,paste0("G:/Sourav/USFS/Revised/Coweeta/RFA_results_all/",RG[s],"/",dur[d]),row.names=FALSE, col.names=FALSE,na = "NaN")
...
write.table(pf,paste0("D:/Sourav/USFS/Revised/Coweeta/RFA_parameters_GEV"),row.names=FALSE, col.names=FALSE,na = "NaN")
