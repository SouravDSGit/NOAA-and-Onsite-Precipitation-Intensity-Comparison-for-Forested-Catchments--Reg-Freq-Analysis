...
% clc
% clear all
% ls=dir(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\textfiles\'])
% file=['01hrs';'02hrs';'03hrs';'06hrs';'12hrs';'24hrs'];
% for q=3:length(ls)
%     site=ls(q).name
%     est=importdata(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\textfiles\',site,'\AMS_est.txt']);
%     est=est';
%     low=importdata(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\textfiles\',site,'\AMS_lower.txt']);
%     low=low';
%     high=importdata(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\textfiles\',site,'\AMS_upper.txt']);
%     high=high';
%     
%     for i=1:6
%         
...
%         
%         dlmwrite(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\Final\',site,'\',file(i,:)],outf,'delimiter','\t');
%         outf=[];
%     end
% end
...
clc
clear all
ls=dir(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\Final\']);
np=1;
file=['01hrs';'02hrs';'03hrs';'06hrs';'12hrs';'24hrs'];
g=[3;5;4];
for q=1:length(g(:,1))
    ...
    for di=1:length(file(:,1))
        duration=file(di,:);        
        rll=importdata(['G:\Sourav\USFS\Revised\Coweeta\RFA_results\',site,'\',duration]);
%         rll=importdata(['G:/Sourav/USFS/Revised/Coweeta/Return_Level_GEV/',site,'/Lmoments/',duration]);
%         rll=[[2, 5, 10, 20, 25,30,35,40,45,50,100]',rll];
        ...
        noaa=importdata(['G:\Sourav\USFS\Revised\Coweeta\NOAA_PIDFs\Final\',site,'\',duration]);
    ...
    xlabel('Duration')
    xlim([0.75 n-0.5])
    set(gca,'TickDir','out','FontName','Sans Serif','FontSize',9);
