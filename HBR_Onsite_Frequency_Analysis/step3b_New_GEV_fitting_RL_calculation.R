...
rm(list = ls())
library(extRemes)
files=list.files("D:/Sourav/USFS/Revised/Hubbard Brook/Annual_Maxima_Series/")
prf=list()
for(m in 1:length(files)){
  print(files[m])
  out=as.data.frame(read.table(paste0("D:/Sourav/USFS/Revised/Hubbard Brook/Annual_Maxima_Series/",files[m]),fill=TRUE))
  out=out[3:47,]
  ...
  x=seq(2,500,by=1)
  rlmom=cbind(x,rlmom)
  #write.table(rlmom, paste0("G:/Sourav/USFS/Revised/Hubbard Brook/Return_Level_GEV/Lmoments/",files[m]),row.names=FALSE, col.names=FALSE)
  parl=ci(fitlmom, alpha = 0.1, type = c("parameter"),which.par, R = 1000)
  #write.table(parl, paste0("G:/Sourav/USFS/Revised/Hubbard Brook/GEV_parameters/Lmoments/",files[m]),row.names=FALSE, col.names=FALSE)
  prf=rbind(prf,cbind(files[m],t(parl[1,]),t(parl[2,]),t(parl[3,])))
}
write.table(prf, paste0("D:/Sourav/USFS/Revised/Hubbard Brook/GEV_parameters_Lmoments"),row.names=FALSE, col.names=FALSE)
####################################### Stationary GEV Fit #####################################
rm(list = ls())
library(eva)
library(ismev)
...
files=list.files("G:/Sourav/USFS/Revised/Hubbard Brook/Annual_Maxima_Series/")

for(m in 1:length(files)){
  print(files[m])
  out=as.data.frame(read.table(paste0("G:/Sourav/USFS/Revised/Hubbard Brook/Annual_Maxima_Series/",files[m]),fill=TRUE))
  out=out[3:47,]
  out=out[complete.cases(out[,2]),]
  colnames(out)=c("Year","AnMax")
  #out[,2]=detrend(out[,2]);
  ...
  write.table(cbind(x,rls), paste0("G:/Sourav/USFS/Revised/Hubbard Brook/Return_Level_GEV/NS_bayesian/RL_s/",files[m]),row.names=FALSE, col.names=FALSE)
  ...
  write.table(dic_out, paste0("G:/Sourav/USFS/Revised/Hubbard Brook/Return_Level_GEV/NS_bayesian/DIC/",files[m]),row.names=FALSE, col.names=FALSE)
 ...
 write.table(outrl, paste0("G:/Sourav/USFS/Revised/Hubbard Brook/Return_Level_GEV/NS_bayesian/RL_ns/",files[m]),row.names=FALSE, col.names=FALSE)
